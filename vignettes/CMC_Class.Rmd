...
```{r echo=FALSE}
generateMAC(folder = "~/CurrentProjects/Matt_MacGMC/CMC_Class/MAC_DATA1/",
            number_to_generate_per_laminate_type = 2000,
            delete_old = TRUE,
            uniform_variation = TRUE,
            n_ply = 24,
            E_fiber_nominal = 330000,
            E_fiber_percent_variation = 15,
            UTS_fiber_nominal = 2000,
            UTS_fiber_percent_variation = 20,
            nu_fiber_nominal = 0.25,
            nu_fiber_percent_variation = 18,
            E_matrix_nominal = 385000,
            E_matrix_percent_variation = 10,
            UTS_matrix_nominal = 500,
            UTS_matrix_percent_variation = 18,
            nu_matrix_nominal = 0.25,
            nu_matrix_percent_variation = 18,
            volume_fraction_nominal = .225,
            volume_fraction_variation = 0.25,
...
```{r echo=FALSE}
runMAC(src_dir = "~/CurrentProjects/Matt_MacGMC/CMC_Class/MAC_DATA1/", dest_dir = "/acp/u/jstuckne/testing2", base_fn = "graphite_epoxy",user = "jstuckne", n_jobs=80)

#pscp jstuckne@cryoterm1:/acp/u/jstuckne/testing2/*.data "C:/Users/jstuckne/OneDrive - NASA/Documents/CurrentProjects/Matt_MacGMC/CMC_Class/MAC_DATA/
```
...
{
  mac_folder <- "~/CurrentProjects/Matt_MacGMC/CMC_Class/MAC_DATA/"
...
res_ss <- pbapply::pblapply(res_data, function(ss){
  getStressStrainStats(strain = ss[,1], stress = ss[,2], uts_linearity_thresh=0.90) #0.9 because 1:max(linearidx) is too long a vector
  #print(length(ss[,1]))
...
save(experiment, file = "~/CurrentProjects/Matt_MacGMC/CMC_Class/parsed.rdata")
}
```
...
```{r}
## load the inputs 
load("~/CurrentProjects/Matt_MacGMC/CMC_Class/parsed_fixed_props.rdata")
...
# Save the parsed data
experiment <- list(res_mac, res_out, res_data, res_ss_props)
save(experiment, file = "~/CurrentProjects/Matt_MacGMC/CMC_Class/parsed_fixed_props1.rdata")
...
keras::save_model_hdf5(model, filepath = "~/CurrentProjects/Matt_MacGMC/CMC_Class/model2_nonxy_06mae_5layer_250epoch.hdf5")
```

```{r}
model <- keras::load_model_hdf5("~/CurrentProjects/Matt_MacGMC/PMC_Class/model1_nonxy_046mae_5layer_250epoch.hdf5")
...
util <- function(d, B, verbose = TRUE, ...){
  ...
  E_fiber = runif(B, 280000, 380000)
  UTS_fiber = runif(B, 1500, 2500)
  nu_fiber = runif(B, 0.2, 0.3)
  E_matrix = runif(B, 350000, 420000)
  UTS_matrix = runif(B, 400, 600)
  nu_matrix = runif(B, 0.2, 0.3)
  VF = runif(B, 0.2, 0.25)
...
ggsave("~/CurrentProjects/Matt_MacGMC/CMC_Class/d=2_nxy=0.jpg")
}
...
{
m$value[m$value == 0] <- min(m$value[m$value>0]) * 0.9
pdf("~/CurrentProjects/Matt_MacGMC/CMC_Class/d=3_nxy=0.pdf")
...
lapply(layups, function(x) {
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ...
    geom_tile(data = zeros, aes(x = rowname, y = colname), fill = "black")
  x = str_replace(x, '/', '_')
  path = sprintf("~/CurrentProjects/Matt_MacGMC/CMC_Class/d=3_nxy=0_%s.jpg", x)
  print(path)
  ggsave(path)
})
